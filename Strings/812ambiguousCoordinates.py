...
def ambiguousCoordinates(S):
    ...
    while i < len(S) - 1:
        j = i + 1
        while j < len(S) - 1: #for decimal point on the x coordinate only
            if S[:i +  1] == str(int(S[:i +  1])) and S[i + 1:j + 1][-1] != "0" and S[j + 1:] == str(int(S[j + 1:])):
                output.append(("({}.{}, {})").format(S[:i +  1], S[i + 1:j + 1], S[j + 1:]))
            j += 1
        p = i + 1
        while p < len(S) - 1: #for decimal on the y coordinate only
            if S[:i +  1] == str(int(S[:i +  1])) and S[i + 1: p + 1] == str(int(S[i + 1: p + 1])):
